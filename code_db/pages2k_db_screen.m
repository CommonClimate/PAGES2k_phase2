...
% temperature with respect to seasonality
year = []; warning off
...
nm  = 1968; % 164 full calendar years from Jan 1850 - Dec 2013
temp = tmp.Xf(1:nm,:);  %temperature anomaly matrix
tim  = tmp.tvec(1:nm,1)+tmp.tvec(1:nm,2)/12-1/12; % fractional year
...
% Extract intra-annual averages
% ============================
nyi = nm/12; %number of instrumental years
temp_ann = nan(nyi,ns);
temp_jja = temp_ann; temp_djf = temp_ann; temp_ama = temp_ann;

for j = 1:ns      % this is slow, but consistent with what was applied to proxies
    disp(['Gridpoint ' int2str(j) '/' int2str(ns)])
   [temp_ann(:,j),tia,~] = intra_annual_avg(temp(:,j),tim,1,12); 
   temp_ama(:,j) = intra_annual_avg(temp(:,j),tim,4,3); 
    ...
    if n_raw_samples(r) >= sample_thresh & n_ann_samples(r) >= sample_thresh
        if resMed(r) <= 5               % hi-res proxies
            ...
            for j = idx_neigh{r}
                ...
                djf(:,j) = hepta_smooth(temp_djf(:,j),fc);
                jja(:,j) = hepta_smooth(temp_jja(:,j),fc);
        ...
        switch method
            ...
            case 'knockoff'  %TODO: include ama screening
                ...
                n_sig_djf(r)= ~isempty(S_djf{r}); % djf
                n_sig_jja(r)= ~isempty(S_jja{r}); % jja
        end
    end
...
note   = 'In each of the screen_* files, indices correspond to MAT, DJF, JJA & AMA respectively';
