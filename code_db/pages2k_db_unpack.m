...
%  OUTPUT:  saved to PAGES2k_v{vers}_unpack.mat'
%
    ...
    if sum(noNaNx)< 2
        ...
        if resMed(r) < 1 % if subannual, coarse-grain to annual resolution
            if gappy(r)  % there are large gaps, proceed segment by segment
                ...
                Xcal = NaN(size(ta)); Xdjf = Xcal; Xjja = Xcal; Xama = Xcal;
                for s = 1:nseg
                    ...
                    [Zcal,tcal,~]    = intra_annual_avg(Xi,tr,1,12,1); % calendar year
                    Xcal(ismember(ta,tcal)) = Zcal;
                    [Zama,tama,~]  = intra_annual_avg(Xi,tr,4,3,1);  % tropical year
                    Xama(ismember(ta,tama)) = Zama;
                    [Zdjf,tdjf,~]  = intra_annual_avg(Xi,tr,12,2,1); % DJF
                    Xdjf(ismember(ta,tdjf)) = Zdjf;
                    Zjja           = intra_annual_avg(Xi,tr, 6,8,1);  % JJA
                    Xjja(ismember(ta,tcal)) = Zjja;
                end
            else
                % interpolate onto regular time grid
                ...
                % intra annual averages
                [Xcal,tcal,~] = intra_annual_avg(Xi,tr,1,12);
                [Xama,tama,~] = intra_annual_avg(Xi,tr,4,3);
                [Xdjf,tdjf,~] = intra_annual_avg(Xi,tr,12,2);
                Xjja          = intra_annual_avg(Xi,tr, 6,8);

                if sum(~ismember(tcal,unique(floor(tc))))~=0
                    % make sure corals are not annualized where there were original NaNs
                    Xcal(~ismember(tcal,unique(floor(tc)))) = nan;
                    Xdjf(~ismember(tdjf,unique(floor(tc)))) = nan;
                    Xjja(~ismember(tcal,unique(floor(tc)))) = nan;
                    Xama(~ismember(tama,unique(floor(tc)))) = nan;
                end
            end
            ...
            if visual
                ...
                plot(tama,Xama,'k-','linewidth',2);
                if resMed(r) < 1
                    plot(tdjf,Xdjf,'b-',tcal,Xjja,'r-');
                    lab{3} = 'DJF'; lab{4} = 'JJA';
                end
                hold off; lab{1} = 'Raw'; lab{2} = 'Annualized'; 
        ...
        if strcmp(archive{r},'ice core')
            ...
            proxy_jja(ismember(year,tcal),r) = Xjja;
            proxy_ama(ismember(year,tama),r) = Xama;
        end
        clear Xcal Xdjf Xjja Xama tcal tdjf tama
    end
end
...
clear gappy interval iWei noNan noNaNx nseg ny segment t T ta ti tmax
