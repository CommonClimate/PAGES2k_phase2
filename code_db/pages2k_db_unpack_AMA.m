...
load(['C:/Users/neukom/Documents/Database/PAGES2k_DB/v2_published/PAGES2k_v' vers '.mat'])
    ...
    if sum(noNaNx)< 2
        ...
        if resMed(r) < 1 % if subannual, coarse-grain to annual resolution
            if gappy(r)  % there are large gaps, proceed segment by segment
                ...
                Xcal = NaN(size(ta)); Xdjf = Xcal; Xjja = Xcal;Xama = Xann;
                for s = 1:nseg
                ...
                [Xdjf,ta_djf,~] = intra_annual_avg(Xi,tr,12,2);
                Xjja            = intra_annual_avg(Xi,tr, 6,8);
                %for ama the output ta needs to be shifted to make the
                %year match the Apr-Dec portion of the windows RN 2016/11/25
                tm=tm-1;
                ...
                if sum(~ismember(ta,unique(floor(tc))))~=0
                    ...
                    Xdjf(~ismember(ta_djf,unique(floor(tc)))) = nan;
                    Xjja(~ismember(ta,unique(floor(tc)))) = nan;
                    Xama(~ismember(tm,unique(floor(tc)))) = nan;
                end
            end
        ...
        if strcmp(archive{r},'ice core')
            ...
            proxy_djf(ismember(year,ta_djf),r) = Xcal;
            proxy_jja(ismember(year,ta),r) = Xcal;
            proxy_ama(ismember(year,ta_ama),r) = Xama;
        else
            proxy_ann(ismember(year,ta),r) = Xcal;
            proxy_djf(ismember(year,ta_djf),r) = Xdjf;
            proxy_jja(ismember(year,ta),r) = Xjja;
            proxy_ama(ismember(year,tm),r) = Xama;
        end
        clear Xcal Xdjf Xjja Xama ta tm
    end
end
...
save(['PAGES2k_v' vers '_unpack.mat'],'-v6')
