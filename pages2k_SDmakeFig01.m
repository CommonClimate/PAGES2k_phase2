...
set(gcf, 'Position', [660 1 727 800])
...
ttl= ['PAGES2k v' versl ' (',int2str(nr) , ' records from ', int2str(ns), ' sites)'];
%suplabel(ttl)
title('a) Archive types',style_t{:});
% legend
hl = legend(h(pind),leg_lbl,'location',[.83 .70 .1 .2],style_l{:});
...
% loop over records
for r = 1:nr
    h(r) = m_line(p_lon(r),p_lat(r),'marker',Graph{p_code(r),2},'MarkerEdgeColor',edgec{r},'MarkerFaceColor',colR(r,:),'linewidth',[1],'MarkerSize',[7],'linestyle','none');
end
% colorbar
